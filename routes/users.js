...
router.get('/', async (req, res) => {
  try {
    ...
    let userList = await Promise.all(users.map(async user => {
      let userRoles = await UserRoles.find({ user_id: user._id }).populate('role_id', 'role_name');
      let roles = userRoles.map(userRole => userRole.role_id.role_name);

      let permissions = new Set();
      roles.forEach(roleName => {
        let rolePermissions = rolePrivileges.roles[roleName.toLowerCase()];
        if (rolePermissions) {
      ...
      return {
        ...user._doc, // This includes the user fields like _id, email, etc.
        roles,
